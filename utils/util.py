...
def readImag2BCHW(path):
    noise_img = cv2.imread(path)
    noise_img = cv2.cvtColor(noise_img, cv2.COLOR_BGR2RGB)
    noise_img = img_as_float(noise_img)
    noise_img = torch.from_numpy(noise_img.transpose((2, 0, 1))).unsqueeze(dim=0)
    return noise_img
...
def mcalculate_psnr(im1, im2, border=0):
    """
    calculate psnr between image1 and image2. all channel.
    :param im1:
    :param im2:
    :param border:
    :return:
...
def batch_PSNR(img, imclean, border=0):
    ...
    for i in range(img.shape[0]):
        PSNR += mcalculate_psnr(imclean[i,:,], img[i,:,], border)
